...
_consts : INT_CONST '+' INT_CONST 
    ...
    */
    ...
    | expr_list_semicolon expression ';' {$$ = append_Expressions($1, single_Expressions($2));}
    ...
    /* empty */ {$$ = nil_Expressions();}
    | expression {$$ = single_Expressions($1);}
    | expr_list_comma ',' expression {$$ = append_Expressions($1, single_Expressions($3)); }
    ;
    ...
    expr_case:
    case  {$$ = single_Cases($1);}
    | expr_case case {$$ = append_Cases($1, single_Cases($2));}
    ;
    ...
    | expression '@' TYPEID '.' OBJECTID '(' expr_list_comma  ')'
    { $$ = static_dispatch($1, $3, $5, $7);}
    | expression '.' OBJECTID '(' expr_list_comma ')'
    { $$ = dispatch($1, $3, $5); }
    | CASE expression OF expr_case ESAC {$$ = typcase($2, $4);}
    | LET let_statement
    {$$ = $2;}
    | OBJECTID '(' error ')' {;}
    |'{' '{'  error '}' '}' {;}
